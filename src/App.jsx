...
import { 
  ...
  ChevronLeft, PlayCircle, Plus, ExternalLink, Zap, RefreshCw, CheckCircle
} from 'lucide-react';
...
export default function App() {
  ...
  const refreshCurrentSection = async () => {
    ...
    await fetchItems(activeSection.id);
    await fetchAllItems();
  ...
  const handleCreate = async (e) => {
    ...
    if (modalType === 'section') {
      ...
      const newItem = { 
        ...
        title, 
        url: mediaUrl, 
        thumbnail: thumbnail || `https://picsum.photos/id/${Date.now() % 1000 + 100}/300/170`,
        progress: 0 
      };
    ...
    return (
      <div className="min-h-screen bg-[#0a0f1c] text-slate-200 p-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex gap-3">
              <button 
                ...
                className="bg-slate-700 hover:bg-slate-600 px-5 py-3 rounded-2xl font-bold flex items-center gap-2 disabled:opacity-70"
              >
              ...
              <button 
                onClick={() => {
                  alert("✅ Telegram Bot is running!\n\nForward any lecture to your channel.\nIt will appear in 'Inbox' automatically.\n\nClick 'Refresh' after forwarding.");
                }} 
          ...
          {/* Netflix-style lecture cards */}
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            {items.length === 0 ? (
              <p className="text-slate-400 col-span-full text-center py-12">No lectures yet. Forward from Telegram or add manually.</p>
            ) : (
              items.map(item => (
                <motion.div 
                  key={item.id}
                ...
                >
                  ...
                  <div className="p-4">
                    <h4 className="font-semibold text-white line-clamp-2 mb-3">{item.title}</h4>
                    <div className="flex justify-between items-center text-xs">
                      <button 
                        onClick={(e) => { 
                          e.stopPropagation(); 
                          updateProgress(item.id, item.progress === 100 ? 0 : 100); 
                        }}
      ...
      <main className="max-w-6xl mx-auto p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {filteredSections.length === 0 ? (
            <p className="col-span-full text-slate-400 text-center py-12">No sections yet. Create one above.</p>
          ) : (
        ...
        >
          ...
          {modalType === 'section' ? (
            ...
            <>
              ...
              />
              <input 
                placeholder="Thumbnail URL (optional)" value={thumbnail} onChange={e => setThumbnail(e.target.value)}
